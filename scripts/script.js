$(document).ready(function () {
    ...
    function initLocationMap() {
        ...
        function checkIframeLoad() {
            try {
                ...
                if (doc && doc.body && doc.body.classList.contains('neterror')) {
                    mapContainer.classList.add('is-iframe-failed');
                }
            } catch (error) {
                // Доступ запрещён — iframe с yandex.ru загрузился.
            }
        }

        iframe.addEventListener('load', checkIframeLoad);
        setTimeout(checkIframeLoad, 2500);
    }
    ...
    function initAppModal() {
        ...
        const $modalText = $('#form-modal-title');
    ...
    const appModal = initAppModal();

    function initOffersButtons() {
        if (!appModal) {
            return;
        }

        $('.offers-button').on('click', function () {
            appModal.openModal('Раздел находится в разработке');
        });
    }
    ...
    function initLocationForm() {
        ...
        function validateLocationForm() {
            ...
            if (!phone) {
                ...
                showFieldError(
                    ...
                    'Введите телефон в формате +7(999) 999-9999'
                );
                isValid = false;
            }
        ...
        function handleCheckoutResponse(response) {
            if (response && Number(response.success) === 1) {
                ...
                if (appModal) {
                    appModal.openModal('Заявка отправлена');
                }
